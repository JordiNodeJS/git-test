...
-- @block
select  pe.nom_planta, ad.nom_adob
from planta_exterior pe, dosi_adob da, adob ad
where pe.nom_planta = da.nom_planta and da.nom_adob=ad.nom_adob
and nom_firma='CIRSADOB'
-- @block
-- misma consulta pero con joins, da el mismo resultado.
select  pe.nom_planta, ad.nom_adob
from planta_exterior pe join dosi_adob da join adob ad
on pe.nom_planta = da.nom_planta and da.nom_adob=ad.nom_adob
and nom_firma='CIRSADOB'
-- @block
...
-- @block
select distinct metode_reproduccio from reproduccio re, exemplar_planta
WHERE re.nom_planta = ep.nom_planta;
...
SELECT DISTINCT CONCAT(nom_popular, '(', quantitat_adob, ')') as cantidad
FROM planta,
      dosi_adob,
      adob
...
FROM planta,
      ...
      adob ad
WHERE nom_firma = 'UOCADOB'
      and da.nom_adob = ad.nom_adob
      and da.nom_planta = nom_cientific;
-- @block
-- con  join
SELECT DISTINCT CONCAT(nom_popular, '(', quantitat_adob, ')') as cantidad
FROM planta JOIN
...
FROM planta,
      reproduccio
WHERE nom_cientific = nom_planta AND floracio = 'Hivern';
...
where (nom_cientic = pe.nom_planta and pe.nom_planta= re.nom_planta
and re.metode_reproduccio = 'Esqueix') or (
      pi.nom_planta = pe.nom_cientific and nom_cientific.re.nom_planta
)
;
